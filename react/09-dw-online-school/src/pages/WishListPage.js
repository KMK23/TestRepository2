...
function WishListPage({}) {
  ...
  const loginUserEmail = JSON.parse(localStorage.getItem("member"));
  ...
  const handleLoad = async () => {
    const resultData = await getData("member", {
      field: "email",
      condition: "==",
      value: loginUserEmail.email,
    });
    // 문서 안에 있는 courseList를 state 에 set 해준다
    setCourseList(resultData.courseList);
  };

  const handleDelete = async (course) => {
    const result = await updateDatas("member", loginUserEmail.docId, course, {
      type: "DELETE",
      fieldName: "courseList",
    });
    handleLoad();
  };
  //useEffect 안에서 handleLoad함수 실행
  useEffect(() => {
    handleLoad();
  }, []);
  return (
